...
const FileRender: FC<Props> = ({ fileExt, fileName, fileUrl, fileSize }) => {
      ...
      <div className="flex items-center justify-start gap-3">
        {isImage ? (
          <img
            src={fileUrl}
            alt={fileName}
            className="w-14 h-14 rounded-xl object-cover aspect-square shrink-0 pointer-events-none"
          />
        ) : (
          <div
          ...
          >
            <p className="text-sm text-white transition-opacity duration-у00 group-hover:opacity-0">
              {fileExt}
            </p>

            <ArrowDownToLine size={22} className="absolute text-white opacity-0 transition-opacity duration-у00 group-hover:opacity-100" />
          </div>
        )}
      </div>
