...
const CheckboxDefault: FC<ICheckboxDefault> = ({
  ...
  return (
    ...
    >
      {/* checkbox */}
      <div
        className={`w-5 h-5 rounded-md flex items-center justify-center ${!isActive ? 'border-2 border-white/40' : ''}`}
      >
        {isActive && <div className="w-5 h-5 bg-accent rounded-md flex items-center justify-center"><Check size={17} /></div>}
      </div>
      <p>{content}</p>
    </div>
  );
